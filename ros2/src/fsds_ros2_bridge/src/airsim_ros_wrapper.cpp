...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : msr::airlib::AirSimSettings::singleton().vehicles)
    {
        ...
        if(UDP_control){
            auto control_cmd_sub = nh_->create_subscription<fs_msgs::msg::ControlCommand>("/fsds/control_command", 1, std::bind(&AirsimROSWrapper::car_control_cb, this, std::placeholders::_1));
        } else {
            auto control_cmd_sub = nh_->create_subscription<fs_msgs::msg::ControlCommand>("/fsds/control_command", 1, std::bind(&AirsimROSWrapper::car_control_cb, this, std::placeholders::_1));
        }
